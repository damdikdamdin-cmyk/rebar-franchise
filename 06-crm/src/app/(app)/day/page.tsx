...
const OPENING = [
  ...
  "Проверить кассу и терминал эквайринга, открыть смену в OS",
  "Сверить остатки витрины с системой (выборочно 5 позиций)",
...
const CLOSING = [
  "Пробить все продажи дня в POS, проверить S/N",
  "Оформить возвраты и уценки, если были",
  "Снять Z-отчёт, сверить наличные с кассой в OS",
  "Убрать устройства в сейф, закрыть смену",
...
export default async function DayPage() {
  ...
  const [store, enrollment] = await Promise.all([
    ...
    prisma.enrollment.findFirst({
      where: { userId: user.id, course: { slug: "seller-day" } },
      include: { course: { include: { lessons: true } }, progress: true },
    }),
  ]);
  const today = new Date().toLocaleDateString("ru-RU", { weekday: "long", day: "numeric", month: "long" });
  const storeHref = store ? `/stores/${store.id}` : "/stores";
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <Card className="mb-6 flex flex-wrap items-center justify-between gap-3 px-5 py-4">
        <div>
          ...
          <p className="mt-1 text-sm">Продажи, склад и касса — в этой же системе</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button asChild size="sm">
            <Link href={`${storeHref}/pos`}>POS</Link>
          </Button>
          <Button asChild size="sm" variant="outline">
            <Link href={`${storeHref}/stock`}>Остатки</Link>
          </Button>
          <Button asChild size="sm" variant="outline">
            <Link href={`${storeHref}/cash`}>Касса</Link>
          </Button>
        </div>
      </Card>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        ...
        <Link href="/learn/seller-day" className="block rounded-lg border border-border bg-card px-4 py-3 text-sm hover:bg-accent">
          ...
          <p className="mt-1">Курс продавца {enrollment ? `· ${courseProgress(enrollment).pct}%` : ""}</p>
        </Link>
      </div>
    </div>
  );
