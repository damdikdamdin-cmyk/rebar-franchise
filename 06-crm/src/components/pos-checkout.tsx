  ...
  const onScan = useCallback(
    (code: string) => {
      const hit =
        ...
        products.find((p) => p.barcode?.endsWith(code) || p.code.endsWith(code));
      if (hit) {
        addProduct(hit);
    ...
    <div className="space-y-4">
      {soldId ? (
        <div className="flex flex-wrap items-center gap-3 border border-border bg-card px-4 py-3">
          <p className="text-sm">Чек оформлен</p>
          <PrintActions
            ...
            actions={[
              ...
              { key: "label", label: "Этикетки товаров", href: `/stores/${storeId}/warranty` },
            ]}
          />
        </div>
      ) : null}
        ...
        <section className="border border-border bg-card">
          <div className="flex items-center gap-2 border-b border-border p-3">
            <Input
              ...
              placeholder="Поиск, код или штрихкод…"
              value={query}
          ...
          {scanMsg ? <p className="border-b border-border px-3 py-1.5 font-mono text-[10px] text-muted-foreground">{scanMsg}</p> : null}
